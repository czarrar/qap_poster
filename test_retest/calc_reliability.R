#' here i will read in the corr dataset (which I still need to get)
#' and then i would do some TRT analysis

#' so we would see for say SNR what's the TRT across sessions
#' i suppose i could also use kendall's W
...
#' ## Calculations
#' 
#' We will compute consistency with kendall's W and reliability with ICC
#' 
#' ### Kendall's W
#' 
#' For each measure and site, we calculate kendall's W.
#+ calc-kendall
qc_anat_ken <- ddply(qc_anat_trt2, .(measure, site), function(x) {
  # matrix: subject x session
  x2  <- acast(x, subject ~ session, value.var = "value")
  res <- kendall(x2, correct=T)
  c(subjects=res$subjects, sessions=res$raters, wt=res$value, pval=res$p.value)
})
...
qc_anat_icc <- ddply(qc_anat_trt2, .(measure, site), function(x) {
  ...
  res <- ICCest(subject, value, data=x, CI.type="S")
  c(subjects=res$N, sessions=res$k, icc=res$ICC, lowerci=res$LowerCI, upperci=res$UpperCI)
})
...
#' ## Calculations (Functional Between-Session)
#' 
#' We will compute consistency with kendall's W and reliability with ICC
#' 
#' ### Kendall's W
...
#' For each measure and site, we calculate kendall's W.
#+ calc-kendall
qc_func_ken_btw <- ddply(qc_func_trt_btw, .(measure, site), function(x) {
  # matrix: subject x session
  x2  <- acast(x, subject ~ session, value.var = "value")
  res <- kendall(x2, correct=T)
  c(subjects=res$subjects, sessions=res$raters, wt=res$value, pval=res$p.value)
...
#' ## Calculations (Functional Within-Session)
#' 
#' We will compute consistency with kendall's W and reliability with ICC
#' 
#' ### Kendall's W
...
#' For each measure and site, we calculate kendall's W.
#+ calc-kendall
qc_func_ken_win <- ddply(qc_func_trt_win, .(measure, site), function(x) {
  # matrix: subject x session
  x2  <- acast(x, subject ~ scan, value.var = "value")
  res <- kendall(x2, correct=T)
  c(subjects=res$subjects, sessions=res$raters, wt=res$value, pval=res$p.value)
})
...
#' For each measure and site, we calculate the ICC.
#+ calc-icc
qc_func_icc_win <- ddply(qc_func_trt_win, .(measure, site), function(x) {
  # matrix: subject x session
  x$subject <- as.factor(x$subject)
  res <- ICCest(subject, value, data=x, CI.type="S")
  c(subjects=res$N, sessions=res$k, icc=res$ICC, lowerci=res$LowerCI, upperci=res$UpperCI)
...
#' save
write.csv(qc_anat_ken, file="qc_anat_kendall_btw_sess.csv")
write.csv(qc_anat_icc, file="qc_anat_icc_btw_sess.csv")
write.csv(qc_func_ken_btw, file="qc_func_kendall_btw_sess.csv")
